...
TARGETS="Lib/I2C Lib/main_def Lib/resources Lib/text_adv Lib/simon ISR_IO/main"
OBJECTS=""
...
if [ "$1" == "clean" ]; then
  ...
  avr-gcc -o ISR_IO.elf $OBJECTS -Wl,-Map="ISR_IO.map" -Wl,--start-group -Wl,-lm -Wl,--end-group -Wl,-Lavrxmega3 -Wl,-LLib -Wl,-Lavr/lib -Wl,--gc-sections -mrelax -mmcu=attiny1617 -B dev/attiny1617 
  avr-objcopy -O ihex -R .eeprom -R .fuse -R .lock -R .signature -R .user_signatures  "ISR_IO.elf" "ISR_IO.hex"
  avr-objcopy -j .eeprom  --set-section-flags=.eeprom=alloc,load --change-section-lma .eeprom=0  --no-change-warnings -O ihex "ISR_IO.elf" "ISR_IO.eep" 
